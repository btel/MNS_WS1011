...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Create the following arrays: integers from 0 to 99 (\texttt{np.arange}), the sequence $\lbrace5.1, 5.2, 5.3, ...,15.0\rbrace$ (\texttt{np.arange}), 5x10 matrix of zeros or ones (\texttt{np.zeros}/\texttt{np.ones}), 5x5 identity matrix (\texttt{np.eye})
        ...
        Multiply the array $A$ by a scalar, mulitply arrays $A$ and $B$ element-wise, calculate a matrix dot product $A$ and $B$ (\texttt{np.dot}).
        \item Multiply element-wise column 1 of matrix A with row 2 of matrix B using slice notation.
    \end{enumerate}
